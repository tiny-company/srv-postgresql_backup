...
### restore postgresql backup from restic restore at RESTORE_RESTIC_TARGET_DIR ###
    for DB in ${RESTORE_POSTGRES_DB_LIST}; do
        ...
        PG_RESTORE_RESULT=$(pg_restore -h ${POSTGRES_HOST} -U ${POSTGRES_USERNAME} -d template1 -j ${RESTORE_PARALELL_THREAD} --clean --if-exists --create --exit-on-error --format=custom $BACKUP_DMP_FILENAME)
        if [ $? -ne 0 ]; then
